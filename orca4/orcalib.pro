...
; 2017.12.09  k.i., s.t.	remove bridge in savefits_p to prevent memory occupation
; 2018.08.26  k.i..		orca_getimg  remove if nimg gt 1
; 2019.04.28  k.i.,k.o.         orcaobs: comment out orca_idle and capturemode_snap, imgs[*,*,i]->imgs[0,0,i], orca_idle after OrcaWait
; 2019.05.03  k.i.,a.m.         sparse keyword
...
function OrcaInit,noDev=noDev0
  ...
  orcadll='C:\Projects\cprog\VS2017\orca4\x64\Debug\orca4.dll'
...
;**************************************************************
function OrcaReset	; not work

  common orca4,orcadll,p,img,noDev

  p_org=p
;  p=OrcaSetParam(regionx=0,regiony=0,width=2048/p.bin,height=2048/p.bin)
  p=OrcaSetParam(regionx=0,regiony=0,width=2048,height=2048)
  messagebox,'ORCA Reset'
  p=p_org
stop
  p=OrcaSetParam(regionx=p.regionx,regiony=p.regiony,width=p.width/p.bin,height=p.height/p.bin)
  return,p

end

;**************************************************************
function orcaobs,file=filename,nimg=nimg,nowait=nowait,noread=noread,grabimg_wait=grabimg_wait,sparse=sparse

  common orca4,orcadll,p,img,noDev,imgs,imgss

  if not keyword_set(nimg) then nimg=1
  if keyword_set(sparse) then m=2 else m=1
...
;t1=anytim2tai(get_systime())
  if not noDev then begin
    if not keyword_set(grabimg_wait) then begin
      r=call_external(orcadll,'GrabImg',nimg*m)
    endif else begin
      r=call_external(orcadll,'GrabImg_Wait',nimg*m)
    endelse
  endif
;print,'GrabImg '+string(anytim2tai(get_systime())-t1)
...
;t1=anytim2tai(get_systime())
  if not noDev then begin
    ...
    ;r=call_external(orcadll,'OrcaWait_CaptureEnd')
    orca_idle  ; <=== 2019.4.28
  endif
;print,'OrcaWait '+string(anytim2tai(get_systime())-t1)
...
;t1=anytim2tai(get_systime())
  ...
  for i=0,nimg-1 do begin
    if not noDev then begin
      r=call_external(orcadll,'GoIdl',i*m,img,timstmp) ; timstmp not word
    endif
    if nimg eq 1 then return,img
    imgs[0,0,i]=img
    ;imgs[*,*,i]=img
  endfor
;print,'GoIdl '+string(anytim2tai(get_systime())-t1)
...
;**************************************************************
function orca_getimg,nimg=nimg,nowait=nowait,idle=idle,sparse=sparse
  ...
  if not noDev and not keyword_set(nowait) then begin
    r=call_external(orcadll,'OrcaWait')
    if keyword_set(idle) then orca_idle
  endif
  ...
  ;if nimg gt 1 and max([p.width,p.height,nimg] ne imgss) then imgs=uintarr(p.width,p.height,nimg)
  if max([p.width,p.height,nimg] ne imgss) then imgs=uintarr(p.width,p.height,nimg)
...
pro orca_handle, ev, wd, p, img1
  ...
  case (ev.id) of
    ...
    wd.SPARSE: begin
      p.sparse=widget_info(wd.SPARSE,/button_set)
      print,'Sparse=',p.sparse
    end
    wd.AUTOSCL:begin
...
function orca_widget,base,p
  ...
  dmy = widget_label(b24, value='Sparse:')
  b24a=widget_base(b24, /row,/ nonex)
  wd.SPARSE= widget_button(b24a,value='', uvalue='SPARSE')
  widget_control,wd.SPARSE,set_button=p.sparse
