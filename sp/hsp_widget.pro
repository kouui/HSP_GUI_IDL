...
;	if (uvalue eq "mtpulse") then wp.mtpulse=strcompress(string(value),/remove_all)
	...
	if (uvalue eq "period") then begin
		...
		if float(value) ge 0.3 then begin
			...
			if wp.empmove eq 1 then begin  ; if already rotating, then stop it
				empstop
				wp.empmove=0
				wait,0.05
			endif
	...
	; every time we change a value in widget, these 3 events will be processed
	; /nowiat : without blocking the thread
	ev_prevstop=widget_event(wd.bt_pren,/nowait)         ; stop preview
	ev_opend=widget_event(wd.opend,/nowait)              ; stop the rotation of turret
	ev_opst=widget_event(wd.bs_wv_op,/nowait)            ; start the rotation of turret ?

	;------ Save -------;
	; GET NOM, currently GET NOM is useless
	; for nf loops : capture nimg files, and then display only the first image
	...
	print,''
	print,'!!FINISH NORMAL OBSERVATION!!'
	print,''
	print,''
	print,''
	print,''
	...
	if (uvalue eq "svp") then begin
		; if waveplate is not rotating, rotate it
		if wp.empmove ne 1 then begin
			empinit,'COM'+wp.com
			empset,1,vm=pm.m1.vm
			wait,0.05
			empstart,1,/CW  ; /CW : Counterlock Wise ?
			wp.empmove=1
			wait,3
		endif
		; `OrcaOutTriggerExposure` will output trigger signal from camera 
	...
	print,''
	print,'!!FINISH POLARIMETRIC OBSERVATION!!'
	print,''
	print,''
	print,''
	print,''
	...
	print,''
	print,'!!FINISH GETING CALIBRATION DATA!!'
	print,''
	print,''
	print,''
	print,''
...
wp={widget_param, $
	...
	svdir:		'D:\data\'+	strmid(time,0,8)+'\',	$	; save directory
	fname:		'fname',	    $		; head of file name
	nf:		    1,		        $		; number of files
	input:		tmp,		    $		; contec cdio input
	output:		tmp,		    $		; contec cdio output
	r_m:		' ',		    $		; RADIUS [arcmin]
	r_s:		' ',		    $		; RADIUS [arcsec]
	...
	com:		'9',		    $		; COM
	empmove:	0,		        $		; motor control: move 1 ; stop 0
	rot_freq:	1.,		        $		; waveplate rotating frequency [Hz]
...
svwp3=wp
wp.motorpulse=MotorPulse(wp)
; The SPAWN procedure spawns a child process to execute a command or series of commands. 
;     commands that could be understood by shell
if is_dir(wp.svdir) eq 0 then spawn,'mkdir '+wp.svdir
...
;------- END :  initialization of GUI ---------------------------

; structure to hold widget object
wd={wd_cdio,	$
	bt_pren:	0l,	$		; ?
	in:		    0l,	$		; contec DIO ?
	out:		0l,	$		; contec DIO ?
	opend:		0l,	$		; contec DIO ?
	bs_wv_op:	0l,	$		; contec DIO ?
	expo:		0l,	$		; exposure time [us]
	gain:		0,	$		; gain ?
	nimg:		0, 	$		;  # of image
	binx:		0, 	$		; Binning X ?
	biny:		0, 	$		; Binning Y ?
...
bs_ob=widget_base(main, /column, /frame)
	...
	bs_sv=widget_base(bs_ob, /column)
		...
		bs_sv2=widget_base(bs_sv, /row)		;20110827
			...
			text=widget_text(bs_sv2,value=wp.wavelength, xsize=6, uvalue='wavelength',/edit,xoffset=100000)
		...
		bs_sv35=widget_base(bs_sv, /row)
			...
			wd.period=widget_text(bs_sv35,value=wp.period, xsize=6, uvalue='period',/edit,xoffset=100000)
...
;------- removed ------------------------------------------------
			...
			wd.com=widget_text(bs_sv35,value=strcompress(wp.com,/remove_all), xsize=6, uvalue='com',/edit,xoffset=100000)
